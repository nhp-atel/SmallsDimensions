<%@ WebHandler Language="C#" Class="SizeReportJsonHandlerLegacy" %>
...
public class SizeReportJsonHandlerLegacy : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        try
        {
            ...
            // Export switch: allow format=csv OR format=xlsx (xlsx will return CSV)
            ...
            if (exportCsv)
            {
                object detailedObj = SizeReportParserLegacy.ParseDetailedTableFromHtml(finalDecodedHtml ?? "");
                ...
                // columns can be object[] or ArrayList depending on serializer
                ...
                // rows can be object[] or ArrayList depending on serializer
                object[] rowsObj = rowsRaw as object[];
                ...
                var columns = colsObj.Select(c => (c ?? "").ToString()).ToList();
                ...
                using (var sw = new StreamWriter(context.Response.OutputStream, Encoding.UTF8))
                {
                    ...
                    // Rows (each ro should be a Dictionary<string, object>)
...
static class SizeReportParserLegacy
{
